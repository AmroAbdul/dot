...
// CONCATENATED MODULE: ./src/storage/migrations.ts

async function idempotentMigrations(_details) {
    await migrateLocalStorageToSyncStorage();
}
async function migrateLocalStorageToSyncStorage() {
    if (!lib["browser"].storage.sync)
        return; // a safeguard for browsers that do not support storage.sync
    const localData = await lib["browser"].storage.local.get();
    if (localData && Object.keys(localData).length > 0) {
        await lib["browser"].storage.sync.set(localData);
        await lib["browser"].storage.local.clear();
        console.log('Data migrated from local storage to sync storage:', JSON.stringify(localData));
    }
}

// CONCATENATED MODULE: ./src/background-listeners-setup.ts


lib["browser"].runtime.onInstalled.addListener(idempotentMigrations);
